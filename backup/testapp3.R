...
SpecexR_app <- function(...) {
                                 ...
                                 mainPanel(
                                   ...
                                   fluidRow( column(3,   sliderInput(inputId = 'wscontro',
                                             ...
                                             column(3,    sliderInput("select2",
                                                                      ...
                                                                      step =1,
                                                                      value = 20)),
                                             column(3, downloadButton('downloadsfall','Downloadshp',class = "butt1") )
                                   ) ,
                                   ...
                                   tabsetPanel(type = "tabs",
                                               ...
                                               tabPanel("LAS polt information",
                                                        sidebarPanel(
                                                          fluidRow(
                                                            column(4, actionButton("dodo", "plot las point cloud")),
                                                            column(4, actionButton("dodo1", "plot single tree point cloud")),
  ...
  server <- function(input, output) {
    ...
    # sflas  <-  reactive({
    #   crowtemp  <-  mult()$crown_polo
    #   crown_pow  <- lapply(crowtemp, function(x){
    #     # opt_output_files(x) <- ''
    #     # crowtemp1  <- crown_metrics(x, func = .stdtreemetrics)
    #     directions_longlat <- st_transform(x, '+proj=longlat +datum=WGS84 +no_defs')
    ...
    #     # spTransform(crowtemp1,sp:: CRS("+proj=longlat +datum=WGS84 +no_defs"))
    #     sf  <- st_as_sf(directions_longlat)
    #     sf33 <- as.data.frame(x)
    #     sf33 <- sf33%>%dplyr:: select(treeID,Z,convhull_area)%>% as.data.frame()
    #     tem <- list(sf,sf33)
    #     names(tem) <- c('sf','sfdata')
    #     return(tem)
    #   })
    #
    #   crown_pow
...
#     })
    ...
    data_ext2  <- reactive({
      ...
      # tesst <-
        # lapply(dsf_list, function(x11) {
        # x22 <- list(unlist(x11))
        # lapply(x22, function(ls11) {
        # lapply(ls11, function(ls222) {
        # tryCatch({
          library(tictoc)
          tictoc::tic("for loop start")
          message(paste0(names(dsf1)))
          tictoc:: tic("processing resample")
          # low <-  terra::rast(dsf1 )
          chm23 <-  terra::resample(chm2,dsf1, method = 'near')
          tictoc::toc()
          tictoc:: tic("processing exact_extract")
          library(exactextractr)
          prec_chm <- exactextractr::exact_extract(chm23, sf, include_xy=T) %>%
          ...
          prec_chm <-  prec_chm %>%
            ...
            area =  sapply(strsplit( rownames(prec_chm),'[_]'), function(x){
              y=x[2]
            }))

          tictoc::toc()
          tictoc::tic("processing data exact_extract")
          library(exactextractr)
          prec_dfs <- exactextractr::exact_extract(dsf1, sf, include_xy=F) %>%
            setNames(paste0(sf$treeID,"_",sf$convhull_area,"_")) %>%
            invoke(rbind,.) %>%
            # mutate(area=sf$convhull_area,
            #                            treeID= sf$treeID
            # )%>%
            dplyr::select(-c(coverage_fraction)) %>%
            as.data.frame()
          message('prec_dfs')
          tictoc::toc()
            ##################
      ...
      # })
      # %>% unlist(recursive = F)  %>% as.data.frame()
          message('rownames(prec_chm)')
      rownames(prec_chm) <-  sapply(strsplit( rownames(prec_chm),'[_]'), function(x){
        y=paste0(x[1],x[3])
      })
      rownames(prec_dfs) <-  sapply(strsplit( rownames(prec_dfs),'[_]'), function(x){
        y=paste0(x[1],x[3])
      })
      message('penultimate step')
      tesst <- prec_dfs[rownames(prec_chm),]
      dat_tes <- cbind(prec_chm,tesst)
      message('The final extraction step')
      dat_tes<- dat_tes %>% as.data.frame()%>%
    ...
    sertree  <-  reactive({
      crowte  <- mult()$crown_polo
      ctg_segmented <- mult()$ctg_segmented
      # crowte  <- list( crowte)
      # crown_se  <- lapply(crowte, function(x){
        ...
        sp::plot(subset3, bg = "white", size = input$poinsize, axis = TRUE, legend = TRUE)
    ...
    rgbplotwithid  <- reactive({
      ...
      if(terra::nlyr(se2) < 3){
        ...
        # lapply(sfff1,function(xyxy){
          ...
          sp::plot(sfff1,add=T,col='white',alpha=0.4)
        ...
        # lapply(sfff1,function(xyxy){
          ...
          p <-ggRGB(se2,  stretch = "hist")+
            geom_sf(data = sfff1, fill=NA,col='orange'   )+
...
#
    ...
    finaldata  <- reactive({
      ...
      if (is.null(dsf1))
        return(NULL)
      data_Jan <- dsf1 %>% invoke(cbind,.) %>% as.data.frame()
    ...
    output$predictPlot3  <- renderPlot({
      ...
      matou_vis2 <- cbind.data.frame(nir[,c(1:2,4,5,6)], nir2)
      matou_vis2 <- matou_vis2 %>% filter(chm > input$heightdata)

      rasterDF <- terra::rast(matou_vis2)

      # plotRGB(rasterDF, stretch = "lin",
      #         r =5, g =16, b =2)
      #
      # spg <- matou_vis2
      # coordinates(spg) <- ~ x + y
