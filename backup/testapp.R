    ...
    mult  <-  reactive({
      ...
      lasdata <-   lapply(las_list, function(ctg){
        expr <- tryCatch({
          ...
          opt_output_files(ctg) <-paste0(tempdir(), input$wscontro,input$hmincor ,"/fdff_{*}")
          opt_chunk_size(ctg) <- 0
          ...
          # plot(chm, col = height.colors(50))
          tictoc:: tic("processing segment_trees")
          opt_output_files(ctg_norm) <-paste0(tempdir(),input$wscontro,input$hmincor )
          algo <- dalponte2016(chm, ttops)
          ctg_segmented <- segment_trees(ctg_norm, algo)
          tictoc:: toc()
          ctg_segmented
