...
SpecexR_app <- function(...) {
                             ...
                             fluidPage(
                               ...
                               sidebarLayout(
                                 sidebarPanel(width = 4,
                                              ...
                                              fluidRow(
                                                column(6,downloadButton("tif_data", label = "Download spectral data")),
                                                column(6, downloadButton("cleaned_data", label = "Download las data"))) ,
                                              # Horizontal line ----
                                              tags$hr(),
  ...
  server <- function(input, output) {
    ...
    data_ext2  <- reactive({
      ...
      subset3 <- mult()$subset3
      # crff  <- crowte$treeID
########### the first way
      # tictoc::tic('project time')
      df <- sf::st_as_sf( crowte$geometry)
      lasukj = readLAS(subset3)

      sf2  <-  crown_polo %>% dplyr:: filter(treeID == input$select2)
    ...
    finaldata  <- reactive({
      ...
      dsf1 <- dsf1 %>% dplyr::select(-ID)
    })

    output$tife_data <- downloadHandler(
      filename = "finaloutput data.rds",
      content = function(file) {readr::write_rds(finaldata(), file)}
    )
