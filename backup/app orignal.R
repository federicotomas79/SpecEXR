...
multi_rasl <- function(las_list,dsf_list,kwsindice,hmin){
  lapply(las_list, function(x){
    expr <- tryCatch({
      ...
      ttops  <- find_trees(nlas_dtm , lmf(ws= kwsindice , hmin = hmin, shape = "circular"))
...
server <- function(input, output) {
  ...
  df_products_upload <- reactive({
    ...
    las_12 <- lapply(inFile$datapath,function(m){
      fdd <- lidR::readLAS(m, filter = "-keep_first -drop_z_below 5 -drop_z_above 50",
                    select = "xyz" )
...
mult  <-  reactive({
  ...
  lasdata <-   lapply(las_list, function(x){
      expr <- tryCatch({
        ...
        nlas_dtm  <- las - dtm
        chm    <- grid_canopy(nlas_dtm , res =0.5, p2r())
        ttops  <- find_trees(nlas_dtm , lmf(ws=input$wscontro, hmin= input$hmincor, shape = "circular"))
        algo   <- dalponte2016(chm, ttops )
        lass   <- segment_trees(nlas_dtm, algo, uniqueness ='incremental')
